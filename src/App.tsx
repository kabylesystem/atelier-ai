...
const mediumLabels: Record<OutputMedium, string> = {
  photo: "Photo",
  video: "Video",
};

const mediumDescriptions: Record<OutputMedium, string> = {
  photo: "A still image prompt with composition, light, camera, and background detail.",
  video: "A motion prompt with action beats, camera movement, timing, continuity, and realism.",
};

function fallbackClarifyingQuestions(medium: OutputMedium): ClarifyingQuestion[] {
  if (medium === "video") {
    return [
      {
        id: "motion",
      ...
      },
      {
        ...
        options: ["Natural held moment", "Funny reaction", "Clean product reveal", "Loopable final frame"],
      },
    ];
  }
...
function enhanceCoreRequest(rawPrompt: string, avatar: Avatar, controls: Controls): string {
  ...
  const details: string[] = [
    `Create a ${controls.aspectRatio} ${controls.medium === "video" ? "video" : "image"} based on this idea: ${idea}.`,
  ];
  ...
  if (controls.medium === "video") {
    details.push(
      "Turn even a vague idea into a complete short video moment with a clear beginning, middle, and end.",
      "Describe natural motion: the subject's micro-expressions, hand movement, posture shifts, clothing movement, background activity, and practical light changes.",
      "Keep identity stable across every frame. No face morphing, no body warping, no sudden costume changes, no drifting background objects.",
    );
  }
...
function buildIntentBlock(intent: Intent, medium: OutputMedium): string {
  if (medium === "video") {
    const base =
      "Create a believable short video, not a still image description. Include subject action, temporal progression, camera movement, scene continuity, and visible cause-and-effect between frames.";

    if (intent === "screen") {
      return `${base} Film a physical screen in the real world with reflections, moire, glass glare, hand tremor, and slight exposure shifts. The UI must remain readable and stable while the camera or subject moves.`;
    }

    if (intent === "avatar") {
      return `${base} Preserve the recurring character's face, proportions, styling logic, voice-less performance, and emotional tone across the full clip. The motion should reveal personality without redesigning the character.`;
    }

    if (intent === "poster") {
      return `${base} Treat the result as a motion poster or campaign clip with intentional graphic hierarchy, legible text, and controlled transitions. Avoid random animated clutter.`;
    }

    if (intent === "product") {
      return `${base} Build the clip around product handling, parallax, reflections, material behavior, and a clean reveal. Preserve product geometry and label logic frame to frame.`;
    }

    if (intent === "edit") {
      return `${base} Treat this as a surgical video edit brief: change only requested elements while preserving camera angle, lighting, timing, identity, and scene physics.`;
    }

    return `${base} Prioritize raw real-world movement, imperfect timing, practical lighting, and ordinary lived-in detail over cinematic polish.`;
  }
...
function compilePrompt(
  ...
  const clarifications = clarificationAnswers.filter((item) => item.answer.trim());
  const clarificationLine = clarifications.length
    ? clarifications.map((item) => `${item.question}: ${item.answer}`).join(" | ")
    : "No extra clarifications provided.";
  ...
  const sections = {
    output_settings: {
      medium: mediumLabels[controls.medium],
    ...
    scene_direction: buildIntentBlock(controls.intent, controls.medium),
    creative_answers: clarifications,
    environment_detail: environmentLine,
    motion_direction: controls.medium === "video"
      ? "Use a 4-8 second clip structure: opening frame establishes the scene, middle beat shows the main action clearly, final beat lands on a natural held moment. Include subtle camera movement, believable body mechanics, stable identity, consistent clothing, background extras continuing their own small actions, and no morphing between frames."
      : "Still image only. Freeze the strongest readable moment with no implied animation artifacts.",
    camera_and_light: {
      shot: controls.shot,
  ...
  const finalPrompt = [
    `${controls.medium === "video" ? "Video" : "Image"} settings: ${controls.aspectRatio}. Intent: ${intentLabels[controls.intent]}. Max length: under ${MAX_PROMPT_CHARS} characters.`,
    "",
    `Core request: ${idea}`,
    "",
    ...
    "",
    `Creative answers: ${clarificationLine}`,
    "",
    controls.medium === "video"
      ? `Motion & timing: ${sections.motion_direction}`
      : `Still-frame direction: ${sections.motion_direction}`,
    "",
  ...
  const score = scorePrompt(rawPrompt, controls, avatar, clarificationAnswers);
  const suggestions = getSuggestions(rawPrompt, controls, clarificationAnswers);

  return { finalPrompt: enforcePromptLimit(finalPrompt), structuredPrompt, score, suggestions };
}
...
function scorePrompt(
  ...
  if (controls.medium === "video") score += 5;
  if (clarificationAnswers.some((item) => item.answer.trim())) score += 10;
...
function getSuggestions(
  ...
  if (!clarificationAnswers.some((item) => item.answer.trim())) {
    suggestions.push("Réponds aux petites questions pour guider le prompt sans toucher aux réglages avancés.");
  }
  ...
  if (controls.medium === "video") suggestions.push("La sortie inclura mouvement, timing, continuité et stabilité du sujet.");
...
export function App() {
  ...
  const [clarifyingQuestions, setClarifyingQuestions] = useState<ClarifyingQuestion[]>(() =>
    fallbackClarifyingQuestions(defaultControls.medium),
  );
  const [clarificationAnswers, setClarificationAnswers] = useState<Record<string, string>>({});
  const [clarifyStatus, setClarifyStatus] = useState<"idle" | "loading" | "error">("idle");
  ...
  const compiled = compilePrompt(rawPrompt, activeAvatar, controls, getClarificationAnswerList());
  ...
  function updateControls(patch: Partial<Controls>) {
    setControls((prev) => ({ ...prev, ...patch }));
    setSmartCompiled(null);
    if (patch.medium) {
      setClarifyingQuestions(fallbackClarifyingQuestions(patch.medium));
      setClarificationAnswers({});
    }
  }

  function getClarificationAnswerList(): ClarificationAnswer[] {
  ...
  function setClarificationAnswer(questionId: string, value: string) {
  ...
  function historyTitle(item: HistoryItem): string {
    ...
    const firstLine = fp
      .split("\n")
      .map((l) => l.trim())
      .find((l) => l && !/^(image|video) settings/i.test(l) && !/^reference/i.test(l));
  ...
  async function refreshClarifyingQuestions() {
      ...
      const response = await fetch("/api/clarify", {
        ...
        body: JSON.stringify({
          ...
          avatar: {
            ...
            role: activeAvatar.role,
            visualMemory: activeAvatar.visualMemory,
          },
        }),
      });
      if (!response.ok) throw new Error("Could not generate questions");
      const data = (await response.json()) as { questions?: ClarifyingQuestion[] };
      const questions = data.questions?.filter((q) => q.id && q.question && q.options?.length) || [];
      setClarifyingQuestions(questions.length ? questions.slice(0, 3) : fallbackClarifyingQuestions(controls.medium));
      setClarificationAnswers({});
      setClarifyStatus("idle");
    } catch (error) {
      console.error(error);
      setClarifyingQuestions(fallbackClarifyingQuestions(controls.medium));
    ...
    try {
      ...
      const response = await fetch("/api/rewrite", {
        ...
        body: JSON.stringify({
          ...
          controls,
          clarificationAnswers: getClarificationAnswerList(),
          localDraft: compiled,
        }),
      });
      if (!response.ok) {
      ...
      data.finalPrompt = enforcePromptLimit(data.finalPrompt || "");
      ...
      <section className="stage">
        <div className="stage-heading">
          ...
          <h1>
            ...
            {page === "output" && (controls.medium === "video" ? "Paste into your video model." : "Paste into your image model.")}
        ...
        {page === "prompt" ? (
          <div className="prompt-page">
            <section className="prompt-canvas">
              <div className="medium-picker" aria-label="Output type">
                {(Object.keys(mediumLabels) as OutputMedium[]).map((medium) => {
                  const Icon = medium === "photo" ? ImageIcon : Clapperboard;
                  return (
                    <button
                      key={medium}
                      type="button"
                      className={controls.medium === medium ? "medium-option active" : "medium-option"}
                      onClick={() => updateControls({ medium })}
                    >
                      <Icon size={18} />
                      <span>
                        <strong>{mediumLabels[medium]}</strong>
                        <small>{mediumDescriptions[medium]}</small>
                      </span>
                    </button>
                  );
                })}
              ...
              <textarea
                ...
                placeholder={
                  controls.medium === "video"
                    ? "Jane dans un kebab avec sa copine, vidéo iPhone, nuit, brut..."
                    : "Jane dans un kebab avec sa copine, photo iPhone, nuit, brut..."
                }
              />
              <div className="prompt-actions">
                <button
                  ...
                  onClick={() => {
                    setPage("direction");
                    void refreshClarifyingQuestions();
                  }}
                >
                  Improve
        ...
        {page === "direction" ? (
          <div className="direction-page">
            <section className="direction-section quick-questions">
              <div className="page-kicker">
              ...
              <p className="mode-hint">
                {controls.medium === "video"
                  ? "Answer the tiny video questions. The prompt will handle motion, timing, camera continuity, and identity stability."
                  : "Answer the tiny image questions. The prompt will handle camera, light, scene detail, and identity stability."}
              </p>
                  ...
                  <article className="question-card" key={question.id}>
                    ...
                    <div className="answer-row">
                      {question.options.map((option) => {
                        const selected = clarificationAnswers[question.id] === option;
                        return (
                          <button
                            key={option}
                            type="button"
                            className={selected ? "answer-chip active" : "answer-chip"}
                    ...
                    <input
                      ...
                      value={
                        question.options.includes(clarificationAnswers[question.id] || "")
                          ? ""
              ...
              <div className="quick-actions">
                <button
                  ...
                  type="button"
                  onClick={refreshClarifyingQuestions}
                  disabled={clarifyStatus === "loading"}
                >
                  {clarifyStatus === "loading" ? "Thinking..." : "New questions"}
                </button>
                {clarifyStatus === "error" ? <span className="inline-warning">Using simple fallback questions.</span> : null}
              </div>
            </section>
                ...
                <div className="advanced-body">
                  <div className="field-grid">
                    <label>
                      Prompt type
                      <select
                        value={controls.intent}
                        onChange={(event) => updateControls({ intent: event.target.value as Intent })}
                      >
                    ...
                    <label>
                      ...
                      <select
                        ...
                        onChange={(event) => updateControls({ aspectRatio: event.target.value })}
                      >
                    ...
                    <label>
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          updateControls({ realism: event.target.value as Controls["realism"] })
                        }
                      >
                    ...
                    <label>
                      ...
                      <input
                        value={controls.shot}
                        onChange={(event) => updateControls({ shot: event.target.value })}
                      />
                      <div className="template-row" aria-label="Device camera presets">
                        {devicePresets.map((preset) => (
                          <button
                            ...
                            onClick={() => updateControls({ shot: preset.spec })}
                            title={preset.spec}
                          >
                            {preset.label}
                          </button>
                    ...
                    <label>
                      ...
                      <input
                        ...
                        onChange={(event) => updateControls({ lighting: event.target.value })}
                      />
                      <div className="template-row">
                        {lightingTemplates.map((template) => (
                          <button key={template} type="button" onClick={() => updateControls({ lighting: template })}>
                            {template}
                          </button>
                        ))}
                      </div>
                    ...
                    <label>
                      ...
                      <input
                        ...
                        onChange={(event) => updateControls({ exactText: event.target.value })}
                  ...
                  <label className="toggle-row">
                    <input
                      ...
                      onChange={(event) => updateControls({ preserveIdentity: event.target.checked })}
                    />
            ...
            <section className="direction-section prompt-runner">
              <button className="next-button" type="button" onClick={runSmartRewrite} disabled={rewriteStatus === "loading"}>
                {rewriteStatus === "loading" ? "Rewriting..." : "Generate prompt"}
                <ChevronRight size={18} />
              </button>
          ...
          <div className="output-page">
            <section className="readiness-band">
              ...
              <div>
                ...
                <strong>
                  {smartCompiled
                    ? `Smart rewrite ready. Copy this into your ${controls.medium === "video" ? "video" : "image"} model.`
              ...
              <div className="reference-copy">
                ...
                <strong>
                  {(activeAvatar.images?.length ?? 0) > 0
                    ? `Attach ${activeAvatar.images!.length} reference image${activeAvatar.images!.length > 1 ? "s" : ""} of ${activeAvatar.name}, then paste the ${controls.medium} prompt below.`
                    : `No reference image set for ${activeAvatar.name}. Add one in the character page so identity stays locked across generations.`}
                </strong>
                <p>
                  Use uploaded references as the source of truth for face/identity. Without them, the character drifts.
                </p>
                {(activeAvatar.images?.length ?? 0) > 0 ? (
            ...
            <section className="reference-band" aria-label="Add result to bank">
              <div className="reference-copy" style={{ width: "100%" }}>
                ...
                <strong>Add the result back to {activeAvatar.name}'s bank.</strong>
                <p>Paste the generated image or best video frame here to grow the reference set so future generations stay consistent.</p>
                <div className="image-bank-add">
                  <input
                    type="url"
                    value={outputBankUrl}
                    onChange={(event) => setOutputBankUrl(event.target.value)}
                    placeholder="Paste generated result URL"
                  />
                  <button
                    type="button"
                    className="upload-btn"
                ...
                <div className="output-head">
                  <div>
                    ...
                    <span className="output-tag">
                      {controls.medium === "video" ? "Video prompt" : "Image prompt"} / {output.finalPrompt.length}/{MAX_PROMPT_CHARS}
                    </span>
                  </div>
                  <button
